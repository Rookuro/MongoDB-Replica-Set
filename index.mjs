...
function createUsersJson(outputOptions) {//Nous mettons un paramètre et par la suite nous allons créer une const pour inclure plusieurs paramètres
    outputOptions.forEach(option => {
        const users = [];

        // Générer des données pour chaque utilisateur
        for (let i = 0; i < option.numUsers; i++) {//Une boucle for on nous allons indiquer dans la déclaration de fonction en tout en bas du code
            const user = {
              ...
              createAt: faker.date.past().toISOString(),//Génére automatiquement une date du passé
            };
        ...
        writeFile(option.outputFile, jsonData, (err) => {
            if (err) {
                console.error(`Erreur lors de l'écriture du fichier ${option.outputFile} :`, err);
            } else {
...
const outputOptions = [
  { numUsers: 100, outputFile: 'users.json'},
];
